...
:- dynamic(object/4). %(OType,OName,OX,OY)
:- dynamic(game_started/1). game_started(false).
...
%SAVE AND LOAD : --------------------------------------------------------------------------------------------
	...
	/*countEnemy( N ) :-
		aggregate( count, X^enemy( X ), N ).
	countObject( N ) :-
		aggregate( count, X^object( X ), N ).*/
		
	countEnemy(X) :-
		findall(N, enemy(N,_,_,_,_), Ns),
		length(Ns, X).
	
	countObject(X) :-
		findall(N, object(N,_,_,_), Ns),
		length(Ns, X).
	
	/*savemultipleE(0).
	savemultipleE(Stream,N,Stream2) :- 
						write(Stream,EType),	write(Stream,'.'),nl(Stream),
						write(Stream,HP),	write(Stream,'.'),nl(Stream),
						write(Stream,EWeapon),	write(Stream,'.'),nl(Stream),
						write(Stream,EX),	write(Stream,'.'),nl(Stream),
						write(Stream,EY),	write(Stream,'.'),nl(Stream),
						NewN is N+1, readmultipleE(Stream,NewN,Stream2).
						
	savemultipleO(0).
	savemultipleO(Stream,N,Stream2) :- 
						write(Stream,OType),	write(Stream,'.'),nl(Stream),
						write(Stream,OName),	write(Stream,'.'),nl(Stream),
						write(Stream,OX),	write(Stream,'.'),nl(Stream),
						write(Stream,OY),	write(Stream,'.'),nl(Stream),
	...
	savegame(File_Name) :-
		...
		player_ammo(AM),
		write(Stream,AM),	write(Stream,'.'),nl(Stream),
		player_position(X,Y),
		write(Stream,X),	write(Stream,'.'),nl(Stream),
		write(Stream,Y),	write(Stream,'.'),nl(Stream),
		player_bag(Size,Inv),
		write(Stream,Size),	write(Stream,'.'),nl(Stream),
		write(Stream,Inv),	write(Stream,'.'),nl(Stream),
		turn(T),
		write(Stream,T),	write(Stream,'.'),nl(Stream),
		threatlvl(TLVL),
		write(Stream,TLVL),	write(Stream,'.'),nl(Stream),
		cur_minimap(CMM),
		write(Stream,CMM),	write(Stream,'.'),nl(Stream),
		enemy_number(EN),
		write(Stream,EN),	write(Stream,'.'),nl(Stream),
		
		countEnemy(Count1),
		write(Stream,Count1),	write(Stream,'.'),nl(Stream),
		
		forall(enemy(EType,HP,EWeapon,EX,EY),
				(
				write(Stream,EType),	write(Stream,'.'),nl(Stream),
				write(Stream,HP),	write(Stream,'.'),nl(Stream),
				write(Stream,EWeapon),	write(Stream,'.'),nl(Stream),
		...
		countObject(Count2),
		write(Stream,Count2),	write(Stream,'.'),nl(Stream),
		
		forall(object(OType,OName,OX,OY), 
				(
				write(Stream,OType),	write(Stream,'.'),nl(Stream),
				write(Stream,OName),	write(Stream,'.'),nl(Stream),
				write(Stream,OX),	write(Stream,'.'),nl(Stream),
...
readmultipleE(Stream,N) :- 
					...
					read(Stream, EX),
					read(Stream, EY),
					asserta(enemy(EType, EHP, EWeapon, EX, EY)),
					NewN is N-1, readmultipleE(Stream,NewN).
readmultipleO(_,0).
readmultipleO(Stream,N) :- 
					read(Stream, OType),
					read(Stream, OName),
					read(Stream, OX),
					read(Stream, OY),
					asserta(object(OType, OName, OX, OY)),
	...
	loadd(File_Name) :-
		...
		enemy_number(EN),
		%gamemap(M),
		%enemy(Type,HP,EWeapon,EX,EY),
		%object(OType,OName,OX,OY),
		
		write('Please sit tight and wait like a good baby.'),nl,
		...
		retract(turn(T)),
		retract(threatlvl(TLVL)),
		retract(cur_minimap(CMM)),
		retract(enemy_number(EN)),
		%retract(gamemap(M)),
		retractall(enemy(_,_,_,_,_)),
		retractall(object(_,_,_,_)),
		write('Successfully reset into clean slate.'),nl,
		%read(Stream, NewM),
		read(Stream, Hp_baru),
		...
		write('Succesfully Read savefile'),nl,
		%asserta(gamemap(NewM)),
		asserta(player_health(Hp_baru)),
		asserta(player_weapon(Weapon_baru)),
		asserta(player_armor(Armor_baru)),
		asserta(player_ammo(NAM)),
		asserta(player_position(NX,NY)),
		asserta(player_bag(NSize,NInv)),
		asserta(turn(NT)),
		asserta(threatlvl(NTLVL)),
		asserta(cur_minimap(NCMM)),
		asserta(enemy_number(NEN)),
		
		%Read ENEMY DATA :
		read(Stream, Count1),
		readmultipleE(Stream,Count1),
		
		%Read OBJECT DATA :
		read(Stream, Count2),
		readmultipleO(Stream,Count2),
	...
	resetmap :- gamemap(M), player_position(X,Y), changemap(M,X,Y,'-',M2), 
				forall(enemy(_,_,_,X2,Y2), changemap(M2,X2,Y2,'-',_)).
				
	setupmap :- gamemap(M), player_position(X,Y), changemap(M,X,Y,'P',M2), 
				forall(enemy(_,_,_,X2,Y2), changemap(M2,X2,Y2,'E',_)).
