...
%CHECK STATUS :
	status :- 
			...
			player_inventory(INV),	write('Inventory          :          '), write(INV),nl,
			write('%%%%%%%%%% STATUS WINDOW %%%%%%%%%'),nl.
...
%Matrix Rules :
  ...
  %RANDOM ENEMY PLACEMENT GROUP ----------------------------------------------
	...
	itemNameList(['medicine', 'tolak angin']).
...
%RULE REPEATER :
	...
	%GAME LOOP :
		...
		setup :- callmultiple(placeRandomEnemy,10), callmultiple(placeRandomObject,10), call(placeRandomPlayer), call(initMM(1,1)).
		game :-
			  write('>> '),
...
%RULE CERITA : -------------------------------------
  ...
  deadzone_expanded :- write('OH NO! Deadzone is expanding. Be careful.'), nl.
	...
	narrate_look  :- write('<<<<<<<<<<----------SYSTEM MESSAGE---------->>>>>>>>>>'),nl,		 		  
							  ...
							  Yleft is Y-1, Yright is Y+1, Xdown is X+1, Xup is X-1,
							  ((at(M,X,Y,Val), Val == 'E')->enemy_center; true),
							  ((at(M,Xup,Y,Val), Val == 'E')->enemy_north; true),
							  ((at(M,Xup,Yright,Val), Val == 'E') -> enemy_northeast; true),
                              ((at(M,X,Yright,Val), Val == 'E') -> enemy_east; true),
							  ((at(M,Xdown,Yright,Val), Val == 'E')-> enemy_southeast;true),
                              ((at(M,Xdown,Y,Val), Val == 'E')-> enemy_south;true),
							  ((at(M,Xdown,Yleft,Val), Val == 'E')->enemy_southwest;true),
							  ((at(M,X,Yleft,Val), Val == 'E')->enemy_west;true),
							  ((at(M,Xup,Yleft,Val), Val == 'E')->enemy_northwest; true),
							  write('<<<<<<<<<<----------*****END.*****---------->>>>>>>>>>'),nl.							  
